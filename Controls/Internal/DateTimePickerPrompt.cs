...
internal partial class DateTimePickerPrompt : Form, IAnimatable
{
	...
	private string getViewTitle()
	{
		return view switch
		{
			DateView.Years => $"{(values[DateView.Years] - (values[DateView.Years] % 12)).Between(DateBox.MinDate.Year, DateBox.MaxDate.Year)} - {(values[DateView.Years] - (values[DateView.Years] % 12) + 11).Between(DateBox.MinDate.Year, DateBox.MaxDate.Year)}",
			DateView.Months => values[DateView.Years].ToString(),
			DateView.Days => $"{new DateTime(values[DateView.Years], values[DateView.Months], 1):MMMM yyyy}",
			_ => string.Empty,
		};
	}
