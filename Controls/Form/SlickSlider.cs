...
public class SlickSlider : SlickControl, IAnimatable
{
	...
	public double Percentage
	{
		get => Values[1]; set
		{
			var c = Values[1] != value.Between(0, 1);
			Values[1] = value.Between(0, 1);
			Invalidate();
			if (c)
			{
	...
	public double PercFrom
	{
		get => Values[0]; set
		{
			var c = Values[0] != value.Between(0, 1);
			Values[0] = value.Between(0, 1);
			Invalidate();
			if (c)
			{
				ValuesChanged?.Invoke(this, new EventArgs());
