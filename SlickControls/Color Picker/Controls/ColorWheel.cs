...
namespace MechanikaDesign.WinForms.UI.ColorPicker
{
	...
	internal partial class ColorWheel : Control
	{
		...
		protected PointF GetColorLocation(Color color) => GetColorLocation(new HslColor(color));
		...
		protected float GetRadius(PointF centerPoint) => Math.Min(centerPoint.X, centerPoint.Y) - (Math.Max(Padding.Horizontal, Padding.Vertical) + (SelectionSize / 2));
		...
		protected void PaintColor(PaintEventArgs e, HslColor color) => PaintColor(e, color, false);
		...
		protected virtual void PaintCurrentColor(PaintEventArgs e) => PaintColor(e, HslColor, true);
