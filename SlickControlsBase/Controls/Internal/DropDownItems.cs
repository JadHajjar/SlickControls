...
namespace SlickControls
{
	internal partial class DropDownItems : SlickControl
	{
		...
		private readonly List<string> listText = new List<string>();
		private readonly List<object> listItems = new List<object>();
		...
		public DropDownItems(Func<object, string> conversion = null, bool fontDropdown = false)
		{
			...
			P_Items.MouseMove += (s, e) => { if (lastMousePoint != Cursor.Position) { lastMousePoint = Cursor.Position; mouseMoved = true; P_Items.Invalidate(); } };
			P_Items.MouseDown += (s, e) => { mouseDown = true; P_Items.Invalidate(); };
			P_Items.MouseUp += (s, e) => { mouseDown = false; P_Items.Invalidate(); };
		}
